...
export class glTFParser {
   ...
   */
  static createMesh(
      asset: glTFAsset, materialFactory?: MaterialFactory, mesh = 0) {
  ...
  parseBuffer(accessor: any) {
    ...
    if (bufferView.byteStride !== undefined && bufferView.byteStride !== 0) {
      size = bufferView.byteStride / componentSize[accessor.componentType] *
              (accessor.count - 1) +
    ...
    return glTFAttribute.from(
        accessor.componentType, componentCount[accessor.type], buffer, offset,
        size, bufferView.byteStride, normalized, accessor.min, accessor.max)
  }

  /**
   * Creates an animation from the specified animation.
   * @param animation The source animation object or index.
   * @param nodes The array of nodes which are potential targets for the
   *     animation.
   */
  parseAnimation(animation: any, nodes: Container3D[]) {
    ...
    for (let channel of animation.channels) {
      ...
      let animationChannel = glTFChannelFactory.create(
          input.buffer, output.buffer, sampler.interpolation || 'LINEAR',
          channel.target.path, nodes[channel.target.node])
      if (animationChannel) {
  ...
  parseMaterial(material?: any) {
    ...
    if (material.occlusionTexture !== undefined) {
      ...
      if (material.occlusionTexture.extensions &&
        ...
        result.occlusionTexture.transform =
            material.occlusionTexture.extensions.KHR_texture_transform
        if (material.occlusionTexture.extensions.KHR_texture_transform
                .texCoord !== undefined) {
    ...
    if (material.normalTexture !== undefined) {
      ...
      if (material.normalTexture.extensions &&
        ...
        result.normalTexture.transform =
            material.normalTexture.extensions.KHR_texture_transform
        if (material.normalTexture.extensions.KHR_texture_transform.texCoord !==
            undefined) {
    ...
    if (material.emissiveTexture !== undefined) {
      ...
      if (material.emissiveTexture.extensions &&
        ...
        result.emissiveTexture.transform =
            material.emissiveTexture.extensions.KHR_texture_transform
        if (material.emissiveTexture.extensions.KHR_texture_transform
                .texCoord !== undefined) {
    ...
    if (pbr?.metallicRoughnessTexture !== undefined) {
      ...
      if (pbr.metallicRoughnessTexture.extensions &&
          pbr.metallicRoughnessTexture.extensions.KHR_texture_transform) {
        result.metallicRoughnessTexture.transform =
            pbr.metallicRoughnessTexture.extensions.KHR_texture_transform
        if (pbr.metallicRoughnessTexture.extensions.KHR_texture_transform
                .texCoord !== undefined) {
          result.metallicRoughnessTexture.texCoord =
              pbr.metallicRoughnessTexture.extensions.KHR_texture_transform
                  .texCoord
    ...
    if (pbr?.baseColorTexture !== undefined) {
      ...
      if (pbr.baseColorTexture.extensions &&
        ...
        result.baseColorTexture.transform =
            pbr.baseColorTexture.extensions.KHR_texture_transform
        if (pbr.baseColorTexture.extensions.KHR_texture_transform.texCoord !==
            undefined) {
   ...
   * Read more about this in discussion at
   * https://github.com/KhronosGroup/glTF/issues/821
   */
  parseMesh(mesh: any) {
    ...
    return <Mesh3D[]>mesh.primitives.map((primitive: any) => {
      return Object.assign<Mesh3D, Partial<Mesh3D>>(
          ...
          {name: mesh.name, targetWeights: weights})
    })
  }
  ...
  parseSkin(skin: any, target: Container3D, nodes: Container3D[]) {
    ...
    return new Skin(
        target, skin.joints.map((joint: number) => <Joint>nodes[joint]))
  }
    ...
    for (let skin of this._asset.descriptor.skins || []) {
      ...
      if (i >= 0) {
        ...
        joint = Object.assign<Joint, Partial<Joint>>(
            new Joint(inverseBindMatrix), {name: node.name})
      }
    }
    let container = joint ||
        Object.assign<Container3D, Partial<Container3D>>(
            new Container3D(), {name: node.name})
    if (node.translation) {
  ...
  parseModel() {
    let nodes = <Container3D[]>this._descriptor.nodes.map(
        (n: any, i: number) => {return this.parseNode(i)})
    ...
    let createHierarchy = (parent: Container3D, node: number) => {
      ...
      if (this._asset.descriptor.nodes[node].skin !== undefined) {
        skin = this.parseSkin(
            this._asset.descriptor.nodes[node].skin, nodes[node], nodes)
      }
